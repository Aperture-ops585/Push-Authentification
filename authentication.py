...
# Generate authentication options for the WebAuthn biometric verification
def auth_options():
    req_id = request.args.get("id") #ID generated by ntfy-PAM Auth 
    ...
    # Convert the authentication options to JSON and return them
...
def auth_verify():
    ...
    try:
        ...
        auth_verification = verify_authentication_response(
            credential=request.json, #Phone provides the biometric assertion JSON
            expected_challenge=challenge, #Unique challenge for this request
            expected_origin=ORIGIN, #Origin of the request
            expected_rp_id=RP_ID, #Relying party ID for security
            credential_public_key=base64url_to_bytes(cred["public_key"]), #Public key stored in the DB
            credential_current_sign_count=cred["sign_count"] #Signature count from the device for security
        )
        
        # Update signature count in DB
        save_credential(cred["id"], cred["public_key"], auth_verification.new_sign_count)
        
        # Publish approval payload to ntfy server response topic
